...
// 2. INICIALIZACIÓN (Cambiamos el nombre a 'database' para evitar conflictos)
const app = initializeApp(firebaseConfig);
const analytics = getAnalytics(app); 
const database = getDatabase(app); // <--- VARIABLE RENOMBRADA
...
// 4. FUNCIONES FIREBASE (Usando la nueva variable 'database')
function saveScore(playerName, score) {
    if (!playerName) playerName = "Jugador Anónimo";
    const scoresRef = ref(database, 'scores'); // <--- USAMOS 'database'
...
function spawnFood() {
    const types = [{ color: "red", effect: "grow" }, { color: "purple", effect: "shrink" }, { color: "orange", effect: "paralysis" }, { color: "blue", effect: "reverse" }];
...
function drawFood() { ctx.fillStyle = food.color; ctx.fillRect(food.x, food.y, box, box); }
function drawSnake() { snake.forEach((segment, i) => { ctx.fillStyle = i === 0 ? "lime" : "green"; ctx.fillRect(segment.x, segment.y, box, box); }); }
...
function moveSnake() {
    ...
    if (snake.some(seg => seg.x === newHead.x && seg.y === newHead.y)) {
        clearInterval(game);
        const name = prompt("Game Over. Tu nombre:");
        saveScore(name, score);
        alert("Puntaje guardado: " + score);
    }
    snake.unshift(newHead);
}

function applyEffect(effect) {
    if (effect === "grow") { const last = snake[snake.length - 1]; snake.push({ x: last.x, y: last.y }); }
    if (effect === "shrink") { if (snake.length > 2) { snake.pop(); snake.pop(); } else if (snake.length > 1) { snake.pop(); } }
    if (effect === "paralysis") { paralyzed = true; showMessage("¡Parálisis!", "orange"); setTimeout(() => { paralyzed = false; }, 3000); }
    if (effect === "reverse") { reversedControls = !reversedControls; showMessage(reversedControls ? "¡Invertido!" : "¡Normal!", "blue"); }
}

function draw() { ctx.clearRect(0, 0, canvas.width, canvas.height); drawFood(); drawSnake(); moveSnake(); drawStatus(); }
function showMessage(text, color) { ctx.fillStyle = color; ctx.font = "30px Arial"; ctx.textAlign = "center"; ctx.fillText(text, canvas.width / 2, canvas.height / 2); }
function drawStatus() { 
    let status = paralyzed ? "Paralizado" : (reversedControls ? "Invertido" : "Normal");
    ctx.fillStyle = "white"; ctx.font = "20px Arial"; ctx.fillText("Estado: " + status, 10, 20); 
}
